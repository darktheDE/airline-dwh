...
- **Bảng:** `dbo.ETL_Watermark` (TableName, Last_Load_Date)
- **Dữ liệu khởi tạo:** Mốc `1900-01-01` cho 3 bảng Fact chính.
...
*   **Bước 2 (Control Flow):** Thêm **Execute SQL Task** (Tên: "Get Watermark").
    *   **SQLStatement:** `SELECT Last_Load_Date FROM ETL_Watermark WHERE TableName = 'Fact_Flight_Transaction'`
...
*   **Bước 4 (Control Flow):** Thêm **Execute SQL Task** sau Data Flow (Tên: "Update Watermark").
    *   **SQLStatement:** `UPDATE ETL_Watermark SET Last_Load_Date = GETDATE() WHERE TableName = 'Fact_Flight_Transaction'`
...
2.  Chạy SSIS Package.
3.  **Kết quả mong đợi:** Kéo toàn bộ dữ liệu từ OLTP vào DWH. Kiểm tra bảng `ETL_Watermark` thấy `Last_Load_Date` đã cập nhật thành thời gian hiện tại.
