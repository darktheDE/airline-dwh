...
## 5. Nhật ký sửa lỗi (Bug Fix Log)

### Lỗi 1: Không tìm thấy Stored Procedure (`Could not find stored procedure 'dbo.usp_ExtractTurnaround'`)
- **Nguyên nhân**: SSIS Package thất bại vì chưa tạo Stored Procedure trong DataBase. Ngoài ra script tạo SP `10_Fix_Fact_Turnaround_AllInOne.sql` bị lỗi cú pháp khi dùng `DATEADD` với kiểu `DATE` trả về từ `DATEFROMPARTS`.
- **Giải pháp**: 
  - Cập nhật script `10_Fix_Fact_Turnaround_AllInOne.sql` thêm hàm `CAST(... AS DATETIME)`.
  - Thực thi lại script để tạo thành công SP `dbo.usp_ExtractTurnaround` tại `Airline_Staging`.

### Lỗi 2: SSIS Package sai Connection Manager
- **Nguyên nhân**: Dù đã tạo SP ở `Airline_Staging`, `Load_Fact_Turnaround.dtsx` ở task `SQL - Extract Turnaround to Staging` vẫn báo lỗi không tìm thấy SP do nó đang trỏ tời Connection Manager của `Airline_OLTP`.
- **Giải pháp**: Mở file `.dtsx` và cập nhật thuộc tính `SQLTask:Connection` thành GUID của `Airline_Staging` (`{CF4880A4-97D0-4B56-A259-F73552DB41EC}`).
